...
% ändere Titel Literatur -> Verweise
\addto{\captionsngerman}{\renewcommand{\refname}{Verweise}}
...
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}
        \draw[red,fill=yellow] (0,0) circle [radius=25pt]
            node[black]{\small Sonne};
        \draw[green,fill=blue] (7,0) circle [radius=15pt]
            node[white]{\small\bfseries Erde};

        \draw[thick,domain=20:340,samples=100,->]
            plot ({7*cos(\x)}, {1.5*sin(\x)});

        \draw[thick]
            (7,0) ++ (67.6:10pt) -- +(67.6:10pt)
            (7,0) ++ (247.6:15pt) -- +(247.6:5pt);

        \draw[->, red, domain=20:340, shift={(7,0)}, shift={(67.6:20pt)}, rotate={-23.4}]
            plot ({-0.5*sin(\x)}, {0.1*cos(\x)});
    \end{tikzpicture}
...
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}
        \draw[red,fill=yellow] (7,0) circle [radius=25pt]
            node[black]{\small Sonne};
        \draw[green,fill=blue] (0,0) circle [radius=15pt]
            node[white]{\small\bfseries Erde};

        \draw[thick,domain=30:330,samples=100,->]
            plot ({7*cos(\x)}, {1.5*sin(\x)});

        \draw[thick]
            (67.6:10pt) -- +(67.6:10pt)
            (247.6:15pt) -- +(247.6:5pt);

        \draw[->, red, domain=20:340, shift={(67.6:20pt)}, rotate={-23.4}]
            plot ({-0.5*sin(\x)}, {0.1*cos(\x)});
    \end{tikzpicture}
...
\begin{figure}[hbt]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \begin{tikzpicture}
            \draw[red,fill=yellow,rotate=23.4] (3,0) circle [radius=15pt]
                node[black]{\small\bfseries S};
            \draw[green,fill=blue] (0,0) circle [radius=10pt]
                node[white]{\small\bfseries E};

            \draw[thick,domain=30:330,samples=100,->,rotate=23.4]
                plot ({3*cos(\x)}, {sin(\x)});
            ...
            \draw[->, red, domain=20:340, shift={(0,15pt)}]
                plot ({-0.3*sin(\x)}, {0.05*cos(\x)});
        \end{tikzpicture}
    ...
    \begin{subfigure}{0.48\textwidth}
        ...
        % TODO
        \begin{tikzpicture}
            \draw[red,fill=yellow] (3,-1) circle [radius=15pt]
                node[black]{\small\bfseries S};

            \draw[thick,domain=360:690,samples=100,->]
                plot ({3*cos(\x)}, {-\x/360 + 1 - sin(\x)});

            \draw[green,fill=blue] (0,0) circle [radius=10pt]
                node[white]{\small\bfseries E};

            \draw[thick,domain=0:360,samples=100]
                plot ({3*cos(\x)}, {-\x/360 + 1 - sin(\x)});
            ...
            \draw[thick]
                ...
                (0,-10pt) -- (0,-15pt);
        \end{tikzpicture}
        \caption{Erdrotation korrigiert (vereinfacht)}
    \end{subfigure}
...
\begin{equation}
    ...
    \label{eq:pneu}
\end{equation}
% Vorschriften anstatt Ergebnissen
...
% Laut Duden tatsächlich mit doppeltem Bindestrich
...
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}[xscale=0.08,yscale=0.8]
        \draw[very thin,gray,xstep=5,ystep=1] (-61, 6.5) grid (61,0);
        \draw[dashed] (0,0) -- (0,6.5);

        \draw[red, thick, domain=-61:61,samples=500]
            plot (\x, {6*exp(-3.5*abs(tan(\x) + 0.05))});

        \draw[->,thick] (-61,0) -- (-61, 6.5)
            node[above] {\small Laufgeschwindigkeit [km/h]};
        \draw[->,thick] (-61,0) -- (61,0)
            node[right] {\small Steigung [$^\circ$]};

        \foreach \i in {0, ..., 6}
            \draw (-61, \i) node[left] {\small \i};

        \foreach \i in {-60, -50, ..., 60}
...
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}
        \draw[color={rgb:red,6;green,2;yellow,1}]
            (1,1) -- (4,1) -- (4, 0.5) -- (5, 0.5) -- (5, 1) --
            (8, 1) node[below] {Gebäude} -- (8, 3) -- (1, 3) -- cycle;

        \draw[->,line width=1pt, color=red]
            (2.5, 0.5) node[below right] {$x_1$} --
            (0.5, 2.5) node[above left] {$x_2$};

        \draw[fill] (2.1, 0.9) circle (1.5pt) node[below left] {$x_{\text{neu}}$};
    \end{tikzpicture}
...
\begin{figure}[htb]
    \centering
    \begin{tikzpicture}
        \draw[line width=1pt] (0,3) -- (0,0) -- (3,0) node[right] {Wand};
        \draw[red,->] (2, 3) node[right] {\tiny Richtung Sonne} -- (0.5, 2.5);
        \draw[red,->] (0.5, 2.5) -- (0.5, 0.5);
        \draw[red,->] (0.5,0.5) -- (0.2,0.5);
        \draw[red,->] (0.2,0.5) -- (0.2,0.1);
        \draw[red,->] (0.2,0.1) -- (0.04,0.1);
    \end{tikzpicture}
...
Auswahl. Im Reiter \enquote{Allgemein} kann man zwischen den beiden Modi \enquote{entlang
der Straßen} und \enquote{Querfeldein} hin und her wechseln, die einzelnen
heruntergeladenen Kartendaten löschen und das Programm beenden. Im zweiten Reiter
Zoom-Modus kann man den Karten-Zoom aktivieren oder den Zoom zurücksetzen. Im dritten
Reiter Beispiele kann man Beispiele laden, zurücksetzen, speichern und löschen.
