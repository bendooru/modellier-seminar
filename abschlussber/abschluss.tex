...
berechnen. Wie in \ref{fig:sonnenmodellroute} dargestellt, ergibt sich dabei wie intuitiv
vielleicht erwartet eine elliptische bis kreisförmige Bahn.

\begin{figure}[ht]
    \centering
    %\includegraphics[width=0.98\textwidth]{pfad/zur/datei}
    PLATZHALTER
    \caption{Ideale Laufroute, <Ort>, <Zeit>}
...
die Funktion eine Höchstgeschwindigkeit von 6\,km/h ausgibt. Es seien nun
$v_{\text{max}}$ die maximale Geschwindigkeit in m/min, $\Delta_t$ die zu laufende Zeit,
$p$ unser Ausgangspunkt und $p_{\text{neu}}$ die wie in \eqref{eq:pneu} bestimmte nächste
Position. Zudem soll die Funktion kart2sph kartesische Koordinaten in ein
Tupel\footnote{Die Länge ist uninteressant.} der entsprechenden Kugelkoordinaten
konvertieren. Dann definiere
\begin{equation*}
...
\begin{thebibliography}{3}
    \bibitem{wp:tobler}
        Wikipedia. \emph{Tobler's hiking function},
